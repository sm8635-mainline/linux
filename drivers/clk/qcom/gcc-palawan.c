...
static struct clk_regmap *gcc_palawan_clocks[] = {
...
static const struct qcom_reset_map gcc_palawan_resets[] = {
...
static struct gdsc *gcc_palawan_gdscs[] = {
...
static const struct regmap_config gcc_palawan_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc gcc_palawan_desc = {
	.config = &gcc_palawan_regmap_config,
	.clks = gcc_palawan_clocks,
	.num_clks = ARRAY_SIZE(gcc_palawan_clocks),
	.resets = gcc_palawan_resets,
	.num_resets = ARRAY_SIZE(gcc_palawan_resets),
	.gdscs = gcc_palawan_gdscs,
	.num_gdscs = ARRAY_SIZE(gcc_palawan_gdscs),
};

static const struct of_device_id gcc_palawan_match_table[] = {
	{ .compatible = "qcom,palawan-gcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_palawan_match_table);

static int gcc_palawan_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gcc_palawan_desc);
	if (IS_ERR(regmap))
	...
	qcom_branch_set_clk_en(regmap, 0x26028); /* GCC_CAMERA_HF_XO_CLK */
	qcom_branch_set_clk_en(regmap, 0x2602c); /* GCC_CAMERA_SF_XO_CLK */
	...
	return qcom_cc_really_probe(&pdev->dev, &gcc_palawan_desc, regmap);
}

static struct platform_driver gcc_palawan_driver = {
	.probe = gcc_palawan_probe,
	.driver = {
		.name = "gcc-palawan",
		.of_match_table = gcc_palawan_match_table,
	},
};

static int __init gcc_palawan_init(void)
{
	return platform_driver_register(&gcc_palawan_driver);
}
subsys_initcall(gcc_palawan_init);

static void __exit gcc_palawan_exit(void)
{
	platform_driver_unregister(&gcc_palawan_driver);
}
module_exit(gcc_palawan_exit);

MODULE_DESCRIPTION("QTI GCC Palawan Driver");
MODULE_LICENSE("GPL");